...
export default function MindsetView({
  ...
  const getMindsetIcon = (iconName: string, id: string) => {
    ...
    switch (id) {
      ...
      case "creative":
        return <Sparkles className={`${iconClass} text-[var(--accent)]`} />;
      case "confidence":
        return <Zap className={`${iconClass} text-[var(--accent)]`} />;
      case "focus":
        return <Target className={`${iconClass} text-[#b5d6c5]`} />;
      case "awakening":
        return <Sun className={`${iconClass} text-[#cebdff]`} />;
      case "vitality":
        return <Activity className={`${iconClass} text-[var(--accent)]`} />;
      default:
  ...
  const getSelectedBadgeColor = (id: string) => {
    switch (id) {
      ...
      default:
        return "border-[var(--accent)]/30 text-[var(--accent)]";
    }
  };
  ...
  return (
    <div className="w-full max-w-5xl mx-auto px-1 flex flex-col gap-8 pb-16">
      ...
      <section className="text-center max-w-3xl mx-auto space-y-4 select-none relative">
        <span className="text-[var(--accent)] font-mono text-xs font-bold tracking-[0.25em] uppercase block">
          Personalization
        </span>
          ...
          return (
            <div
              ...
              className={`md:col-span-8 rounded-2xl p-8 md:p-10 flex flex-col justify-between relative overflow-hidden cursor-pointer transition-all duration-400 border min-h-[280px] ${
                isSelected
                  ? "border-[var(--accent)] bg-[var(--accent)]/5 shadow-[0_0_30px_-5px_rgba(var(--accent-rgb),0.15)]"
                  : "bg-white/[0.03] border-white/10 hover:bg-[#161618]/60 hover:border-[var(--accent)]/20"
              }`}
            >
        ...
        {mindsets.filter(m => m.id === "confidence").map((item) => {
          ...
          return (
            ...
            >
              ...
              <div className={`absolute inset-0 bg-radial-at-c from-[var(--accent)]/5 via-transparent to-transparent opacity-0 transition-opacity duration-700 pointer-events-none ${isSelected ? "opacity-100" : ""}`} />
              ...
              <div className="text-center md:text-left flex-1 space-y-1">
                <div className="flex flex-col md:flex-row md:items-center gap-1.5">
                  ...
                  {isSelected && (
                    <span className={`w-fit md:ml-2 font-mono text-[9px] font-bold px-2 py-0.5 rounded border uppercase tracking-wide bg-[var(--accent)]/10 ${getSelectedBadgeColor(item.id)}`}>
                      Selected Focus
                    </span>
                  )}
                </div>
              ...
              <div className="flex gap-1.5 select-none py-1 animate-pulse">
                <span className={`w-1.5 h-1.5 rounded-full ${isSelected ? "bg-[var(--accent)]" : "bg-zinc-600"}`} />
          ...
          return (
            <div
              ...
              className={`md:col-span-4 rounded-xl p-7 md:p-8 flex flex-col items-center text-center cursor-pointer transition-all duration-400 border ${
                isSelected
                  ? "border-[var(--accent)] bg-[var(--accent)]/5 shadow-[0_0_30px_-5px_rgba(var(--accent-rgb),0.15)] scale-[1.02]"
                  : "bg-white/[0.03] border-white/10 hover:bg-[#161618]/60 hover:border-[var(--accent)]/20"
              }`}
            >
              <div className="mb-4">
      ...
      <section className="mt-8 flex flex-col items-center gap-3.5 select-none">
        <button
          ...
          className="bg-[var(--accent)] text-[#0b0b0c] hover:shadow-[0_0_35px_rgba(var(--accent-rgb),0.45)] hover:bg-[var(--accent-hover)] font-mono text-[10px] font-bold uppercase tracking-[0.25em] px-12 py-4 rounded transition-all duration-300 active:scale-95 cursor-pointer"
        >
          Begin Ritual
        </button>
